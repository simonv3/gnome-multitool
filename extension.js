...
const mathJs = Me.imports.node_modules.mathjs.dist.math;
const math = mathJs.math;
...
const WikipediaSearchProvider = new Lang.Class({
    ...
    _on_key_release: function(object, event) {
        ...
        let isCharacter = Utils.get_unichar(symbol);

        if(symbol === Clutter.BackSpace && query.name === 'wikipedia') {
            this._block_search_trigger = true;
            this._wikipedia_display.remove_suggestion();
            this.show_message(_("Enter your query"));
        } else if(isCharacter && query.name === 'wikipedia') {
            if(query.lang) {
        ...
        } else if (query.name === 'currency') {
          let evalled;
          try {
            evalled = math.eval(query.term);
          } catch (e) {
            // do nothing
          }
          this._block_search_trigger = true;
          this._wikipedia_display.remove_suggestion();
          const message = _("Result: %s").format(evalled);
    ...
    _parse_query: function(terms_string) {
        ...
        let multitoolKeywords = [{
          ...
          processRegexp: function (result, regexp, terms_string) {
            let matches = regexp.exec(terms_string);
            let math = matches[1];
            if(!Utils.is_blank(math)) result.term = math.trim();

            return result;
          }
        }, {
          'regex': "^(%s|%s-(.*?)) (.*)".format(keyword, keyword),
          'result': {
            name: 'wikipedia',
            lang: '',
            query: false,
        ...
        const results = multitoolKeywords.map(tool => {
          let regexp = new RegExp(tool.regex);

          // if the regex doesnt match the terms string, we don't
          // do anything for this query
          if (!regexp.test(terms_string)) return tool.result;

          tool.result.query = true;

          return tool.processRegexp(tool.result, regexp, terms_string);
        }).filter(result => {
          return result.query;
        });

        // theoretically there should only be one result?
        return results[0];
    },
    ...
    _create_results_window: function (message, callback) {
    ...
    _start_search: function(term, lang) {
        ...
        const message = _("Searching for '%s'...").format(term);
        const that = this;
        this._create_results_window(message, function () {
          that._search(term);
          that._wikipedia_client.lang = lang;
        });
    },
